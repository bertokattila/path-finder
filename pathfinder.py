...
def distance(vertex1, vertex2):
    return math.sqrt(pow(abs(vertex1.coordinates[0] - vertex2.coordinates[0]), 2) + pow(abs(vertex1.coordinates[1] - vertex2.coordinates[1]), 2))
    
def dijkstra(start, end, verticies):
    graph = copy.deepcopy(verticies)
    for key in graph:
    ...
    while not is_empty(graph):
        ...
        for neighbour in vertex.neighbours:
            alternative_distance = vertex.distance_from_start + distance(vertex, neighbour)
